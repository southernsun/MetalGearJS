...
// southernsun/MetalGear. The disassembly (.asm sources + room_images, READ) is expected as a
// sibling at ../MetalGear; the exported web assets (WRITE) live in THIS repo's web/assets.
// Override the disassembly location with the MG_ROM_DIR environment variable.
internal static class RomPaths
{
    // Disassembly root (the folder containing data/rooms.asm). Order: $MG_ROM_DIR, then walk up
    // from the exe (legacy in-tree), then the sibling ../MetalGear (or a MetalGear child of an
    // ancestor).
    public static string ResolveRomDir()
    {
        ...
        throw new DirectoryNotFoundException(
            ...
            "as a sibling so it sits at ../MetalGear, or set the MG_ROM_DIR environment variable.");
    }
