...
`constants/Enums.asm`). For this slice the owned set is the handgun (weapon) and, as items, the
cardboard box, the oxygen tank, and at least one **keycard** (`SELECTED_CARD1`) so keycard doors are
usable. A selection input SHALL change the current weapon/item. (Browsers reserve the function keys, so
the binding MAY use number keys / a dedicated key — a documented divergence.) No inventory UI is
included.
