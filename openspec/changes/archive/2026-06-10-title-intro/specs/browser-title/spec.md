# browser-title — the boot sequence
...
the title. The logo art SHALL be the decoded ROM graphic, not a recreation.
...
While the Konami logo or the swoop is playing, ANY control SHALL skip to the finished
title — the logo parked instantly with SFX 0x4A ("Menu logo stops") and the texts shown
(`GoToMenu`/`DrawMenuNow`). On the finished title, only Fire 1 / Fire 2 (Space or M —
`ChkAnykeyStart` bits 0x30, Banks0123.asm:10632) SHALL start the game; other keys do
nothing. The attract demo the ROM would run after idling is NOT ported (no demo-input
system) — the title idles instead, a documented divergence.
...
- **THEN** the game starts in room 0 as today
