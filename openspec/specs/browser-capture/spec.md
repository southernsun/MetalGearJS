...
## Purpose
The capture flow: the once-only trigger in room 8, the scripted DON'T MOVE / YOU ARE
CAPTURED scene, the prison transfer with the equipment flagged removed, the punched-out
cell walls, and the equipment-bag recovery with its bugged-transmitter consequence.
## Requirements
...
exported cluster — a `?capture` dev hook places Snake in the zone on the bottom corridor,
the room's real approach from room 12; a documented divergence until the cluster
expansion.)
...
unskippable); the music mutes, the screen fades out at the ROM pacing (`FadeOutColors`
steps only when TickCounter & 3 == 0 — the full fade is ~28 iterations), and Snake is
moved to the prison.

#### Scenario: The scene

- **WHEN** the capture triggers
- **THEN** both guards play their script with both texts, the screen fades slowly to
  black, and play resumes in the prison cell
...
`ChkPrisonWalls` (logic/doors/opendoor.asm:286). Picking up the equipment bag (pickup 34,
room 168) SHALL run `RecoverEquipment` (logic/items.asm:295): `EquipRemoved` cleared,
`EquipBagTaken` set, text 62 shown (the Western pickup gate's one permitted description),
and the TRANSMITTER added to the inventory — the bag is bugged. Room 164 is Grey Fox's
cell (a REAL `RoomsPrisoner` room, rescue text 59); its own wall drops into basement room
54, with 57's punch door reaching the bag.

#### Scenario: Escape and recovery

- **WHEN** Snake punches open the cell wall, crosses Grey Fox's cell and the basement,
  and takes the bag
...
menu (`ChkDropTransmitter`, logic/menuequipment.asm:238 — Fire, or Enter as this port's
confirm key) SHALL consume it and clear the flag, letting the alarm end normally again.
